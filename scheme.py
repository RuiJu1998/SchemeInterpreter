...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        lamb = LambdaProcedure(target.second, expressions.second, env)
        env.define(target.first, lamb)
        return target.first
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    symnbol, value = nil, nil
    while bindings is not nil:
        check_form(bindings.first, 2, 2)
        symnbol = Pair(bindings.first.first, symnbol)
        value = Pair(bindings.first.second.first, value)
        bindings = bindings.second
    eval = lambda x: scheme_eval(x, env)
    value = value.map(eval)
    check_formals(symnbol)
    return env.make_child_frame(symnbol, value)
    # END PROBLEM 15
...
def scheme_optimized_eval(expr, env, tail = False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            result = first.eval_call(rest, env)
            # END PROBLEM 20
    return result
